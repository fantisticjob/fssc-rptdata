...
import com.longfor.fsscreportdb.ods.mapper.OdsZjzcAccountsExtendMapper;
 ...
 */
@Service
public class OdsZjzcAccountsExtendServiceImpl extends ServiceImpl<OdsZjzcAccountsExtendMapper, OdsZjzcAccountsExtend> implements IOdsZjzcAccountsExtendService {
	
	private static final  String STATUS="status";

    private final Logger log = LoggerFactory.getLogger(OdsZjzcAccountsExtendServiceImpl.class);
		...
		try {
			do {
				...
				if(token.get(CommonConstant.ACCESSTOKEN)!=null 
					...
					buffer.append(properties.getProperties("zjzczhkzb"));
				...
				if("200".equals(String.valueOf(datas.get("resultcode")))) {
					Collection<OdsZjzcAccountsExtend> object = stringToDate( datas.get("records"));
					...
					if(!batch) {
						log.error("资金自查账户扩展表数据湖取数保存失败！报错条数为{}",total);
						json.put(STATUS,"0");
						return  json;
					}
			...
			log.error("资金自查账户扩展表数据湖取数保存失败!{}",e.toString());
			json.put(STATUS, 0);
		}
		return  json;
	}
	public List<OdsZjzcAccountsExtend>  stringToDate(Object object) {
		JSONArray array = JSONArray.parseArray( object.toString());
		ArrayList<OdsZjzcAccountsExtend> list = new ArrayList<>();
		for (int i = 0; i < array.size(); i++) {
			Object o = array.get(i);
			OdsZjzcAccountsExtend parseObject = JSON.parseObject(o.toString(),OdsZjzcAccountsExtend.class);
