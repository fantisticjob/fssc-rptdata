...
import com.longfor.fsscreportdb.ods.mapper.OdsZjzcAccountsAppliesMapper;
 ...
 */
@Service
public class OdsZjzcAccountsAppliesServiceImpl extends ServiceImpl<OdsZjzcAccountsAppliesMapper, OdsZjzcAccountsApplies> implements IOdsZjzcAccountsAppliesService {
	
	private static final  String STATUS="status";
	
    private final Logger log = LoggerFactory.getLogger(OdsZjzcAccountsAppliesServiceImpl.class);
		...
		try {
			do {
				...
				if(token.get(CommonConstant.ACCESSTOKEN)!=null
					...
					buffer.append(properties.getProperties("zjzczhsqb"));
				...
				if("200".equals(String.valueOf(datas.get("resultcode")))) {
					Collection<OdsZjzcAccountsApplies> object = StringToDate( datas.get("records"));
					...
					if(!batch) {
						log.error("资金自查账户申请表据湖取数保存失败！报错条数为{}",total);
						json.put(STATUS,"0");
						return  json;
					}
			...
			log.error("资金自查账户申请表据湖取数保存失败!{}",e.toString());
			json.put(STATUS, 0);
		}
		return  json;
	}
	public List<OdsZjzcAccountsApplies>  StringToDate(Object object) {
		JSONArray array = JSONArray.parseArray( object.toString());
		ArrayList<OdsZjzcAccountsApplies> list = new ArrayList<>();
		for (int i = 0; i < array.size(); i++) {
			Object o = array.get(i);
			OdsZjzcAccountsApplies parseObject = JSON.parseObject(o.toString(),OdsZjzcAccountsApplies.class);
