...
import com.longfor.fsscreportdb.ods.mapper.OdsZjzcaccountsMapper;
...
public class OdsZjzcaccountsServiceImpl extends ServiceImpl<OdsZjzcaccountsMapper, OdsZjzcAccount> implements IOdsZjzcaccountsService {
		...
		try {
			do {
				...
				if(token.get(CommonConstant.ACCESSTOKEN)!=null 
					...
					buffer.append(properties.getProperties("zjzczhb"));
				...
				if("200".equals(String.valueOf(datas.get("resultcode")))) {
					Collection<OdsZjzcAccount> object = StringToDate( datas.get("records"));
					...
					if(!batch) {
						log.error("资金自查账户表数据湖取数保存失败！报错条数为{}",total);
						json.put(STATUS,"0");
						return  json;
					}
			...
			log.info("资金自查账户表接口返回总数量为{}",total);
			json.put(STATUS, 200);
		} catch (Exception e) {
			log.error("资金自查账户表数据湖取数保存失败!{}",e.toString());
			json.put(STATUS, 0);
		}
		return  json;
	}
	public List<OdsZjzcAccount>  StringToDate(Object object) {
		JSONArray array = JSONArray.parseArray( object.toString());
		ArrayList<OdsZjzcAccount> list = new ArrayList<>();
		for (int i = 0; i < array.size(); i++) {
			Object o = array.get(i);
			OdsZjzcAccount parseObject = JSON.parseObject(o.toString(),OdsZjzcAccount.class);
