...
import com.longfor.fsscreportdb.ods.mapper.OdsZdSwglzzszbmxMapper;
...
public class OdsZdSwglzzszbmxServiceImpl extends ServiceImpl<OdsZdSwglzzszbmxMapper, OdsZdSwglzzszbmx> implements IOdsZdSwglzzszbmxService {
		...
		try {
			do {
				...
				if(token.get(CommonConstant.ACCESSTOKEN)!=null 
					...
					buffer.append(properties.getProperties("swglzzszbmx"));
				...
				if("200".equals(String.valueOf(datas.get("resultcode")))) {
					List<OdsZdSwglzzszbmx> list = StringToDate( datas.get("records"));
	...
	public List<OdsZdSwglzzszbmx>  StringToDate(Object object) {
		JSONArray array = JSONArray.parseArray( object.toString());
		ArrayList<OdsZdSwglzzszbmx> list = new ArrayList<OdsZdSwglzzszbmx>();
		for (int i = 0; i < array.size(); i++) {
			Object o = array.get(i);
			OdsZdSwglzzszbmx parseObject = JSON.parseObject(o.toString(),OdsZdSwglzzszbmx.class);
