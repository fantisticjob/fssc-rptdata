...
import com.longfor.fsscreportdb.ods.mapper.OdsZdNcviufolhgxMapper;
...
public class OdsZdNcviufolhgxServiceImpl extends ServiceImpl<OdsZdNcviufolhgxMapper, OdsZdNcviufolhgx> implements IOdsZdNcviufolhgxService {
		...
		try {
			do {
				...
				if(token.get(CommonConstant.ACCESSTOKEN)!=null 
					...
					buffer.append(properties.getProperties("ncviufolhgx"));
				...
				if("200".equals(String.valueOf(datas.get("resultcode")))) {
					List<OdsZdNcviufolhgx> object = StringToDate(datas.get("records"));
	...
	public List<OdsZdNcviufolhgx>  StringToDate(Object object) {
		JSONArray array = JSONArray.parseArray( object.toString());
		ArrayList<OdsZdNcviufolhgx> list = new ArrayList<>();
		for (int i = 0; i < array.size(); i++) {
			Object o = array.get(i);
			OdsZdNcviufolhgx parseObject = JSON.parseObject(o.toString(),OdsZdNcviufolhgx.class);
