...
import com.longfor.fsscreportdb.ods.mapper.OdsZdSwglqsMapper;
...
public class OdsZdSwglqsServiceImpl extends ServiceImpl<OdsZdSwglqsMapper, OdsZdSwglqs> implements IOdsZdSwglqsService {
		...
		try {
			do {
				...
				if(token.get(CommonConstant.ACCESSTOKEN)!=null 
					...
					buffer.append(properties.getProperties("swglqs"));
				...
				if("200".equals(String.valueOf(datas.get("resultcode")))) {
					List<OdsZdSwglqs> list = StringToDate( datas.get("records"));
	...
	public List<OdsZdSwglqs>  StringToDate(Object object) {
		JSONArray array = JSONArray.parseArray( object.toString());
		ArrayList<OdsZdSwglqs> list = new ArrayList<>();
		for (int i = 0; i < array.size(); i++) {
			Object o = array.get(i);
			OdsZdSwglqs parseObject = JSON.parseObject(o.toString(),OdsZdSwglqs.class);
