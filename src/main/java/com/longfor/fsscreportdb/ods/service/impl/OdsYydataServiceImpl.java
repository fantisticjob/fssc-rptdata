...
import com.longfor.fsscreportdb.ods.mapper.OdsYydataMapper;
...
public class OdsYydataServiceImpl extends ServiceImpl<OdsYydataMapper, OdsHzYydata> implements IOdsYydataService {
		...
		try {
			do {
				...
				if(token.get(CommonConstant.ACCESSTOKEN)!=null 
					...
					buffer.append(properties.getProperties("yydata"));
				...
				if("200".equals(String.valueOf(datas.get("resultcode")))) {
					List<OdsHzYydata> object = StringToDate( datas.get("records"));
	...
	public List<OdsHzYydata>  StringToDate(Object object) {
		JSONArray array = JSONArray.parseArray( object.toString());
		ArrayList<OdsHzYydata> list = new ArrayList<>();
		for (int i = 0; i < array.size(); i++) {
			Object o = array.get(i);
			OdsHzYydata parseObject = JSON.parseObject(o.toString(),OdsHzYydata.class);
