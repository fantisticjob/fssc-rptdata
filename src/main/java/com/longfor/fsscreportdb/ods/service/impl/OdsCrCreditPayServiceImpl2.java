...
public class OdsCrCreditPayServiceImpl2 extends ServiceImpl<OdsCrCreditPayMapper, OdsCrCreditPay> implements IOdsCrCreditPayService2 {
	
    private final Logger log = LoggerFactory.getLogger(OdsCrCreditPayServiceImpl2.class);
	...
	@Override
	public JSONObject saveList(String startTime, String endTime){
		
		
		log.info("信用超期付款2 开始了");
		boolean flag =true;
		Integer total=0;
		Integer count=0;
		int errorCount = 0;
		...
		try {
			...
			log.info("信用超期付款2DELTETE条数：{}" , deleteCount);
			...
			do {
				...
				if("200".equals(String.valueOf(datas.get("resultcode")))) {
					List<OdsCrCreditPay> list = StringToDate(datas.get("records"),startTime,endTime);
					...
					if(!list.isEmpty()) {
						...
						if(!batch) {
							log.error("信用超期付款2表数据湖取数保存失败！报错条数为{}",total);
							json.put(STATUS,"0");
							return  json;
						}
			...
			log.info("信用超期付款2 结束了");
			json.put(STATUS, 200);
		} catch (Exception e) {
			log.error("信用超期付款2表数据湖取数保存失败!{}",e.toString());
			json.put(STATUS, 0);
		}
		return  json;
	}
	public List<OdsCrCreditPay>  StringToDate(Object object,String startTime, String endTime) {
			...
			if( "先付款后审核".equals(f_sfxzf)  &&  "已支付".equals(f_fkzt) ) {
				...
				if(StringUtils.isNotBlank(jrgx_time) ) {
					...
					try {
						...
						map.put("endTime", StringUtil.getDateConversion2(endTime));
						map.put("startTime", StringUtil.getDateConversion2(startTime));
						...
						log.error("信用超期付款2时间转换异常!{}",e.toString());
					}
