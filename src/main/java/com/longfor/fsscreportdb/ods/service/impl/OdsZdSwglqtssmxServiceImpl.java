...
import com.longfor.fsscreportdb.ods.mapper.OdsZdSwglqtssmxMapper;
...
public class OdsZdSwglqtssmxServiceImpl extends ServiceImpl<OdsZdSwglqtssmxMapper, OdsZdSwglqtssmx> implements IOdsZdSwglqtssmxService {
		...
		try {
			do {
				...
				if(token.get(CommonConstant.ACCESSTOKEN)!=null 
					...
					buffer.append(properties.getProperties("swglqtssmx"));
				...
				if("200".equals(String.valueOf(datas.get("resultcode")))) {
					List<OdsZdSwglqtssmx> object = StringToDate(datas.get("records"));
			...
			log.error("增值税纳税申报表主表数据湖取数保存失败!{}",e);
			json.put(STATUS, 0);
		}
		return  json;
	}
	public List<OdsZdSwglqtssmx>  StringToDate(Object object) {
		JSONArray array = JSONArray.parseArray( object.toString());
		ArrayList<OdsZdSwglqtssmx> list = new ArrayList<>();
		for (int i = 0; i < array.size(); i++) {
			Object o = array.get(i);
			OdsZdSwglqtssmx parseObject = JSON.parseObject(o.toString(),OdsZdSwglqtssmx.class);
