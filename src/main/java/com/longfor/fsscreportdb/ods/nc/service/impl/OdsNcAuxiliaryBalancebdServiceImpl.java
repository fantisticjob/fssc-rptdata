...
import com.longfor.fsscreportdb.ods.nc.mapper.OdsNcAuxiliaryBalancebdMapper;
...
public class OdsNcAuxiliaryBalancebdServiceImpl extends ServiceImpl<OdsNcAuxiliaryBalancebdMapper, OdsNcAuxiliaryBalancebd> implements IOdsNcAuxiliaryBalancebdService {
			...
			for(int i=0 ; i<orgCodeList.size(); i++) {
				...
				for(int j = 0; j<kemuCodeList.size(); j++) {
					...
					if(json.get("data") !=null ) {
						...
						ArrayList<OdsNcAuxiliaryBalancebd> list = new ArrayList<OdsNcAuxiliaryBalancebd>();
						for (int k= 0; k < object.size(); k++) {
							OdsNcAuxiliaryBalancebd vo = JSONObject.toJavaObject(object.get(k), OdsNcAuxiliaryBalancebd.class);
	...
	@Override
	public List<OdsNcAuxiliaryBalancebd> getAllBean(Integer count, Integer total) {
		String sql="SELECT *" + 
				"" + 
				"  FROM (SELECT ROWNUM AS rowno, t.*" + 
				"" + 
				"          FROM ODS_NC_AUXILIARY_BALANCE_BD t" + 
				"" + 
				"           where ROWNUM <= "+total+") table_alias" + 
				"" + 
				" WHERE table_alias.rowno >"+count;
		return dao.getListOdsNcAuxiliarybdBalance(sql);
	}
	
	@Override
	public List<OdsNcAuxiliaryBalancebd> getAllbyOrgCode(String orgCode) {
		String sql="SELECT * from ODS_NC_AUXILIARY_BALANCE_BD where ORGCODE='" + orgCode + "'";
		return dao.getListOdsNcAuxiliarybdBalance(sql);
	}
}
